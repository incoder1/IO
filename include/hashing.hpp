...
public:
	...
	static inline uint32_t hash(const uint8_t* key, std::size_t size) noexcept IO_NO_INLINE {
...
// Use murmur3 when on 32-bit instruction set
template<typename T>
inline std::size_t hash_bytes(const T* bytes,std::size_t count) noexcept {
	return detail::murmur3::hash( reinterpret_cast<const uint8_t*>(bytes), count*sizeof(T) );
}
#else
// Use CityHash when on 64-bit instruction set
template<typename T>
inline std::size_t hash_bytes(const T* bytes,std::size_t count) noexcept {
	return detail::cityhash::hash( reinterpret_cast<const uint8_t*>(bytes), count*sizeof(T) );
}
