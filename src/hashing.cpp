...
static uint32_t hash_over_24(const uint8_t* s, std::size_t len) noexcept
{
    ...
    uint32_t a0 = ror32(fetch_32(s + len - 4) * c1, 17) * c2;
    uint32_t a1 = ror32(fetch_32(s + len - 8) * c1, 17) * c2;
    uint32_t a2 = ror32(fetch_32(s + len - 16) * c1, 17) * c2;
    uint32_t a3 = ror32(fetch_32(s + len - 12) * c1, 17) * c2;
    uint32_t a4 = ror32(fetch_32(s + len - 20) * c1, 17) * c2;
    ...
    do {
        uint32_t a0 = ror32(fetch_32(s) * c1, 17) * c2;
        uint32_t a1 = fetch_32(s + 4);
        uint32_t a2 = ror32(fetch_32(s + 8) * c1, 17) * c2;
        uint32_t a3 = ror32(fetch_32(s + 12) * c1, 17) * c2;
