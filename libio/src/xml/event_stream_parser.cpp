...
static constexpr const char LEFTB = '<';
static constexpr const char RIGHTB = '>';
static constexpr const char SRIGHTB = ']';
static constexpr const char QNM = '"';
static constexpr const char APH = '\'';
static constexpr const char EM = '!';
static constexpr const char SOLIDUS = '/';
static constexpr const char HYPHEN =  '-';
static constexpr const char COLON =  ':';
static constexpr const char ES = '=';
...
document_event event_stream_parser::parse_start_doc() noexcept
{
	...
	if(nullptr != j) {
		...
		standalone = is_true(i);
		if( !standalone &&  is_false(i) ) {
			assign_error(error::illegal_prologue);
			return document_event();
		}
		i = stop + 1;
...
void event_stream_parser::skip_chars() noexcept
{
	...
	for(unsigned int i = detail::unsign( next() ); error_state_ok() ; i = detail::unsign( next() ) ) {
		switch(i) {
		case ULEFTB:
			sb_clear();
			sb_append( LEFTB );
...
{
	switch( detail::unsign(scan_buf_[0]) ) {
