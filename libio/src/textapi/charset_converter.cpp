...
s_pump charset_converting_channel_pump::create(std::error_code& ec,const s_read_channel& src, const charset* from, const charset* to, byte_buffer&& buff) noexcept
{
	s_pump ret;
	byte_buffer cvb = byte_buffer::allocate(ec, buff.capacity()  * 2 );
	if(!ec) {
	    s_charset_converter cvt = charset_converter::open(ec, from, to);
	    if(!ec) {
		    auto px = new (std::nothrow) charset_converting_channel_pump( src, std::forward<byte_buffer>(buff), std::move(cvb), std::move(cvt) );
		    if(nullptr == px)
			    ec = std::make_error_code(std::errc::not_enough_memory);
		    else
			    ret.reset(px, true);
...
s_pump charset_converting_channel_pump::create(std::error_code& ec,const s_read_channel& src,const charset* from, const charset* to, std::size_t out_buffer_size) noexcept
{
    s_pump ret;
    byte_buffer buff = byte_buffer::allocate(ec, out_buffer_size);
    if(!ec)
        ret = create(ec, src, from, to, std::move(buff) );
    return ret;
}

charset_converting_channel_pump::charset_converting_channel_pump(const s_read_channel& src, byte_buffer&& rb, byte_buffer&& cvb, s_charset_converter&& cvt) noexcept:
    buffered_channel_pump( src, std::forward<byte_buffer>(rb) ),
