...
int main()
{
	...
	check_system_error(ec);
	code_cnvtr cvt = code_cnvtr::open(ec,code_pages::UTF_16LE, code_pages::UTF_8);
	check_system_error(ec);

    s_read_channel src = conv_read_channel::open(ec,  sf.open_for_read(ec), std::move(cvt), charset_conv_rate::twice_less );
	check_system_error(ec);

	s_write_channel dst = to.open_for_write(ec, write_open_mode::overwrite);
	check_system_error(ec);
	// write a UTF-8 BOM to destination file first
	write_some(dst, io::utf8_bom::data(), io::utf8_bom::len() );
